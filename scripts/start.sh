...
if find ../src -name "*.java" > sources.txt && javac -d ../bin -cp "../lib/*" @sources.txt; then
    echo "OK."
else
    echo "Error durante la compilación. Revisa el código fuente."
    exit 1
fi
...
#	java -cp ../bin:../../LSim-libraries/* recipes_service.Phase1 $*
	...
	FILE="f_$i"
		if [ $2 -le 4 ]; then
			...
			konsole --hold -e java -cp ../bin:../lib/* recipes_service.Server $* &
		else
